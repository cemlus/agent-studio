...
//     // --- FIX: Proper MediaSource and SourceBuffer cleanup ---
//     if (sourceBufferRef.current) {
//         try {
//             sourceBufferRef.current.abort();
//         } catch (e) {
//             console.warn("Could not abort SourceBuffer, it might be detached.", e);
//         }
//         sourceBufferRef.current = null;
//     }
...
//         mediaSourceRef.current.endOfStream()
//       } catch (e) {
//         console.warn("Could not end MediaSource stream, it might be already closed.", e);
//       }
//     }
//     mediaSourceRef.current = null
//     audioQueueRef.current = []
...
//   const processAudioChunk = useCallback(
//     (audioData: Uint8Array) => {
//       // --- FIX: Add checks for MediaSource and SourceBuffer state ---
//       if (!sourceBufferRef.current || !mediaSourceRef.current || mediaSourceRef.current.readyState !== "open") {
//         console.warn("MediaSource not ready or closed, queuing audio chunk.")
//         audioQueueRef.current.push(audioData)
//         return
//       }
//       // --- END FIX ---
...
//         endCall()
//       }
//     }, { once: true }) // FIX: Use once: true for sourceopen
...
//         setVoiceState((prev) => ({ ...prev, isListening: true }))
//       }
//     }, { once: true }) // FIX: Use once: true for sourceended
//   }, [processAudioChunk, connectionStatus, endCall])
...
//           case LiveConvoWebsocketEvents.Interruption:
//             // --- FIX: Proper cleanup for interruption ---
//             if (sourceBufferRef.current) {
//                 try {
//                     sourceBufferRef.current.abort();
//                 } catch (e) {
//                     console.warn("Could not abort SourceBuffer during interruption.", e);
//                 }
//                 sourceBufferRef.current = null;
//             }
//             if (mediaSourceRef.current && mediaSourceRef.current.readyState === "open") {
...
//       }
//     },
//     [processAudioChunk, setupAudioStream, startBackgroundAudio, endCall, agent.agent.greetingMessage]
//   )
...
//       endCall()
//     }
//   }, [handleWsMessage, agent.agentId, endCall])


//   // Start call
//   const startCall = async (): Promise<void> => {
//     // FIX: Setup audio stream first before connecting to WS
//     setupAudioStream()
//     connectToWebSocket() 
//     setConnectionStatus("connecting")
//     setIsCallActive(true)
...
//           <div className="flex items-center justify-between text-sm text-gray-600">
//             <div className="flex items-center gap-4">
//               <span>Voice: {agent.tts.voiceId}</span>
//               {agent.agent.backgroundSound !== "none" && <span>Background: {agent.agent.backgroundSound}</span>}
//             </div>
//             <div className="flex items-center gap-2">
//               <div className="w-2 h-2 bg-green-500 rounded-full animate-pulse" />
